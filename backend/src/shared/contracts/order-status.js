 ...
 * one that exposes the whole list to an administrator. Parking the list in any
 ...
 *
 * It was omitted because no admin action produces it — the checkout route
 * writes it and markOrderPaid() clears it. But leaving it out did not make it
 * unreachable, it made it *unrepresentable*: an order sitting in it could be
 * moved to any of the other four and never moved back, so one stray click
 * destroyed the only record that money was still owed. A status the database
 * can hold and the API cannot express is a one-way door.
 *
