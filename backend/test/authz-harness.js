...
// requireAdmin/requireCustomer, the actual login route. Only @supabase/
// supabase-js is replaced, with an in-memory fake holding a handful of
// obviously-fictional rows.
//
// That is the point: the live Supabase project must not be written to, and a
// read-only probe cannot exercise the paths that matter most — signing in as
// an admin, an admin PATCHing another customer's order, a guest checkout
// that names an administrator's email. Those need writes and need an admin
// session, so they get a fake database instead of a real one.
// =============================================================================
...
const db = {
    ...
    user_profiles: [
        ...
        // Deliberately has no orders and no address. DELETE /api/customers/:id
        // refuses a customer who has ordered, so without a row like this the
        // suite could only ever prove the refusal and never the deletion.
