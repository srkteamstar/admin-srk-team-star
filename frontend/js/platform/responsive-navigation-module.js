(function () {
    ...
    const css = `
        ...
        @media (max-width:767px){
            ...
            .srk-store-shell,.srk-admin-shell{display:block!important;width:100%!important;min-width:0!important}
            /* These two are overlaid drawers at this width, and both scroll:
               store.html's sidebar is overflow-y-auto in its own markup and
               the admin one is taller than a phone. overscroll-behavior stops a
               drag past their end being handed to the page behind — the same
               rule .srk-mobile-panel above carries, and the same one
               scroll-lock-module.js publishes as .srk-scroll. */
            .srk-store-sidebar,.srk-admin-sidebar{position:fixed!important;top:0;left:0;bottom:0;z-index:90!important;width:min(84vw,300px)!important;height:100dvh!important;overflow-y:auto!important;overscroll-behavior:contain;transform:translateX(-105%);transition:transform .25s ease;background:#fff!important;box-shadow:12px 0 35px rgba(18,23,15,.18)}
            .srk-store-sidebar[data-open="true"],.srk-admin-sidebar[data-open="true"]{transform:translateX(0)}
            .srk-store-main,.srk-admin-main{width:100%!important;min-width:0!important}
            ...
            .srk-admin-main>header{padding-left:1rem!important;padding-right:1rem!important}
            .srk-admin-main>header .w-1\\/3{width:auto!important}
            #dynamic-view{min-width:0!important}
            #main-content{padding:1.25rem!important;min-width:0!important}
            #details-drawer{position:absolute!important;inset:0!important;width:100%!important;max-width:none!important;z-index:60!important}
    ...
    // a phone, the store and admin sidebars, which the stylesheet above turns
    // into off-canvas drawers at the same width and easing.
    //
    ...
    function offCanvasShell(shell, sidebar, main, kind) {
        ...
        label.textContent = kind === 'admin' ? 'Admin navigation' : 'Store navigation';
    ...
    function init() {
        ...
        if (adminNav) {
            const sidebar = adminNav.closest('aside');
            offCanvasShell(sidebar?.parentElement, sidebar, sidebar?.nextElementSibling, 'admin');
        }
