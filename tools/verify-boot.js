 ...
 *    nobody finds until the first request reaches it — which, for an admin
 *    route or a webhook, can be weeks. Loading all of them makes that a boot
 *    failure instead.
 *
 ...
 *    character, or a route somebody added without saying so.
 *
 *    The second direction earns its keep here in a way it did not in a single
 *    repository: it is what keeps the storefront's routes OUT of this process.
 *    The two applications were one, they still share core/ and shared/ by
 *    copy, and the temptation to bring a public controller back across to
 *    "just read the catalogue" is real. Doing so fails this check before it
...
/**
 * The label a non-literal route is reported under. Every route in this
 * application is a literal path — the storefront's regex-matched legal shell
 * did not come across — so this is a passthrough with a guard rather than a
...
function main() {
    ...
    const expected = new Set(contract.routes);

    const missing = [...expected].filter(r => !actual.has(r)).sort();
    const unexpected = [...actual].filter(r => !expected.has(r)).sort();

    console.log(`verify-boot: ${actual.size} route(s) exposed; ${contract.routes.length} in the contract.`);
    ...
    if (missing.length || unexpected.length) {
        ...
        if (unexpected.length) {
            ...
            console.error('  under "added_in_hash2" — with a reason in the file, not silently.');
        }
        console.error('');
        process.exit(1);
    }
